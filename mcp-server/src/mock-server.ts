	...
	try {
		switch (name) {
			case 'get_current_stock': {
				const { style, colour, sku, minStock, maxStock } = args as any;
				...
				if (colour) {
					filtered = filtered.filter((item) =>
						item.colour.toLowerCase().includes(colour.toLowerCase())
					);
				}
					...
					content: [
						{
							...
							text:
								...
								`Results:\n${filtered
									.map(
										(item) =>
											`- ${item.style} ${item.colour} (${item.sku}): ${item.remaining.toFixed(2)} yards remaining`
									)
									.join('\n')}`
						}
					]
			...
			case 'search_inventory': {
				...
				if (stockQuery.style) {
					filtered = filtered.filter((item) =>
						item.style.toLowerCase().includes(stockQuery.style.toLowerCase())
					);
				}
