...
export class FabricDatabase {
	private db: ReturnType<typeof createPool>;

	constructor(connectionString: string) {
		this.db = createPool({ connectionString });
	}

	async getCurrentStock(query: StockQuery = {}): Promise<StockResponse> {
		const {
			style,
			colour,
			sku,
      ...
      `;
			...
			let rows = result.rows as TStyleColour[];
	...
	async getStockHistory(sku: string, days: number = 30): Promise<any[]> {
		try {
			const sqlQuery = `
        SELECT 
      ...
      `;
			...
			throw new Error(
				`Failed to get stock history: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
		}
	}

	async getLowStockItems(threshold: number = 10): Promise<TStyleColour[]> {
		try {
			const sqlQuery = `
        SELECT 
          sc.id, 
      ...
      `;
			...
			return result.rows as TStyleColour[];
		} catch (error) {
			throw new Error(
	...
	async getStockByCategory(): Promise<any[]> {
      ...
      `;
			...
			throw new Error(
				`Failed to get stock by category: ${error instanceof Error ? error.message : 'Unknown error'}`
			);
		}
	}
