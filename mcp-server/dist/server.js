        ...
        switch (name) {
                    ...
                    content: [
                        {
                            ...
                            text: `Current stock query results:\n\n` +
                                ...
                                `Results:\n${result.results
                                    .map((item) => `- ${item.style} ${item.colour} (${item.sku}): ${(item.remaining || 0).toFixed(2)} yards remaining`)
                                    .join('\n')}`
                        }
                    ]
                };
            ...
            case 'get_stock_history': {
                ...
                const historyText = history
                    .map((day) => `${day.date}: ${day.cut_length.toFixed(2)} yards cut (${day.cut_count} cuts)`)
                    .join('\n');
                return {
                    content: [
                        {
            ...
            case 'get_stock_by_category': {
                ...
                const categoriesText = categories
                    .map((cat) => `${cat.style}: ${cat.total_colours} colours, ${(cat.total_stock || 0).toFixed(2)} total yards`)
                    .join('\n');
                return {
                    content: [
                        {
