...
const UpdateHold = ({ hold, colourStyleId, refetchQueries, onComplete }) => {
  ...
  const [length, setLength] = useState(hold.length);
  const [notes, setNotes] = useState(hold.notes);
  const [owner, setOwner] = useState(hold.owner);
  const [expires, setExpires] = useState(
    moment(hold.expires).format("YYYY-MM-DD")
  );
  ...
  return (
    <Mutation mutation={MutationUpdateHold} refetchQueries={refetchQueries}>
      {(updateHold, { loading, error }) => (
        <div>
          {loading && <Loading />}
