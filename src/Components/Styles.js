...
class Styles extends Component {
  ...
  renderStyle = (style) => <div key={style.id}>{style.name}</div>;
  ...
  render() {
    ...
    return (
      <div>
        <div>{styles.map(this.renderStyle)}</div>
        <input
          type="text"
          id="name"
          value={style.name}
        ...
        <input
          ...
          onChange={this.handleChange.bind(this, "thickness")}
        />
        <input
          type="text"
          id="patternWidth"
          value={style.patternWidth}
          onChange={this.handleChange.bind(this, "patternWidth")}
        />
        <input
          type="text"
          id="patternHeight"
          value={style.patternHeight}
          onChange={this.handleChange.bind(this, "patternHeight")}
        />
...
export default withApollo(
  compose(
    graphql(QueryGetStyles, {
      fetchPolicy: "network-only",
      props: ({ data: { getStyles = [] } }) => ({
        styles: getStyles,
      }),
    }),
    graphql(MutationCreateStyle, {
      props: (props) => ({
        createStyle: (style) => {
          return props.mutate({
